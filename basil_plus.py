...
def basil_plus_training(nodes, G, S, rounds, epochs=1, test_loader=None):
    N = len(nodes)
    assert N % G == 0, "Number of nodes must be divisible by number of groups G"
    group_size = N // G
    models = []

    for g in range(G):
        print(f"\n=== Training Group {g + 1}/{G} ===")
        group_nodes = nodes[g * group_size:(g + 1) * group_size]
        trained_models, _ = basil_ring_training(group_nodes, rounds, epochs, test_loader)
        models.extend(trained_models)
