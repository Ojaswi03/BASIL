...
def batchify(dataset, batch_size, dataset_choice='m'):
    """
    Convert dataset to list of (X_batch, y_batch) numpy arrays.
    """
    data_loader = []
    X, y = [], []
...
def plot_accuracy_curve(accs, attack_type, S, dataset_choice, output_dir="diagrams"):
    """
    Plot Basil accuracy curve (with Round 0 baseline included).
    """
    os.makedirs(output_dir, exist_ok=True)
    folder_name = f"{dataset_choice}_{attack_type}_S{S}"
    folder_path = os.path.join(output_dir, folder_name)
    os.makedirs(folder_path, exist_ok=True)

    plt.figure()
    plt.plot(range(len(accs)), accs, label="Basil (Avg)", color="red")
    plt.title(f"Basil Test Accuracy - {dataset_choice.upper()} - Attack: {attack_type}, S={S}")
    plt.xlabel("Round (k)")
    plt.ylabel("Test Accuracy")
    plt.grid(True)
    plt.legend()
    plt.savefig(os.path.join(folder_path, f"{attack_type}_basil_curve.png"))
    plt.close()


def run_experiment(dataset_choice, num_nodes, S, rounds, epochs,
                   iid, use_acds, use_basil_plus, attack_type,
                   attacker_ids, hidden_start, batch_size=64, seed=42):
    ...
    # === Apply ACDS if non-IID ===
    if not iid and use_acds:
        print("Applying ACDS for anonymous data sharing...")
        shared_indices_map = acds_share(list(range(num_nodes)),
                                        node_indices,
                                        alpha=0.05, H=5,
                                        dummy_batch=None, G=1)
    ...
    # === Round 0 evaluation before training ===
    avg_acc, worst_acc, _ = evaluate_all(nodes, test_loader)
    print(f"[Round 0] Average Test Accuracy: {avg_acc:.4f}, Worst Benign: {worst_acc:.4f}")

    # === Run training ===
    if use_basil_plus:
        num_groups = int(num_nodes // S)
        trained_models, accs = basil_plus_training(nodes, num_groups, S, rounds, epochs, test_loader)
    else:
        trained_models, accs = basil_ring_training_with_attack(nodes, rounds, epochs,
                                                               test_loader,
                                                               attack_type,
                                                               attacker_ids,
                                                               hidden_start_round=hidden_start)

    # Prepend Round 0 accuracy to curve
    accs = [avg_acc] + accs

    # === Final evaluation ===
    final_avg, final_worst, acc_list = evaluate_all(nodes, test_loader)
    print("\n=== Experiment Completed ===")
    print(f"Final Average Accuracy: {final_avg:.4f}")
    print(f"Final Worst Benign Accuracy: {final_worst:.4f}")
    print(f"Per-node Accuracies: {acc_list}")

    plot_accuracy_curve(accs, attack_type, S, dataset_choice)
...
if __name__ == '__main__':
    ...
    attack_type = input("Attack type (none/gaussian/sign_flip/hidden): ").strip().lower()
    attacker_ids = []
    if attack_type != "none":
        attacker_input = input("Enter attacker node IDs separated by commas (e.g., 2,5,7): ").strip()
        if attacker_input:
            attacker_ids = [int(x) for x in attacker_input.split(",")]

    hidden_start = 20
    if attack_type == "hidden":
        hidden_start = int(input("Enter round to start hidden attack (default=20): ").strip() or 20)

    run_experiment(dataset_choice, num_nodes, S, rounds, epochs,
                   iid, use_acds, use_basil_plus, attack_type,
                   attacker_ids, hidden_start)
